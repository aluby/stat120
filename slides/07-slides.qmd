...
```{r}
ggplot(penguins, aes(x = body_mass_g, y = flipper_length_mm)) + 
  geom_point(fill = "#f5f190", shape = 21, size = 4) +
...
Predictions ($\widehat{y}_i$): the points on the line for each $x$ value
...
ggplot(broom::augment(penguins_lm), aes(x = body_mass_g, y = flipper_length_mm)) + 
  ...
  geom_point(size = .5) + 
  geom_smooth(method = "lm", se = FALSE) + 
  geom_point(aes(y = .fitted), size = 1, col = "black") 
```
...
Step 3. Create the scatterplot using the augmented dataset (you can also use {esquisse} for this)
...
## What do we ideally want to see? 

```{r}
set.seed(0928)
tibble(
  .resid = rnorm(150, sd = 2),
  .fitted = rnorm(150, mean = 20, sd = 5)
) |>
